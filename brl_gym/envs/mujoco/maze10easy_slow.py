...
class Maze10EasySlow(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
        self.agent_bid = 0
        ...
        self.fullpath = os.path.join(asset_dir, "assets", 'maze10_soft_slow.xml')
        mujoco_env.MujocoEnv.__init__(self, self.fullpath, 5) # originally 30
    ...
    def _sense(self):
        ...
        # Noisy distance
        noise_scale = np.linalg.norm(goal_dist[self.target]) * 2.0 #/ (1.8*np.sqrt(2))
    ...
    def viewer_setup(self):
        ...
        self.viewer.cam.lookat[0] += -.2        # x,y,z offset from the object (works if trackbodyid=-1)
        self.viewer.cam.lookat[1] += -0.0
        self.viewer.cam.lookat[2] += 3.5
