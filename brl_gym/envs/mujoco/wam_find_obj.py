...
class WamFindObjEnv(robot_env.RobotEnv):
    ...
    def _env_setup(self, initial_qpos):
        ...
        self.site_poses = [self.sim.data.get_site_xpos('bookcase:pos{}'.format(x)) for x in range(4)]
        print("x", self.site_poses)
        ...
        for _ in range(100):
            ...
            self.sim.step()
            # print("body pos", self.sim.model.body_pos[22])

        utils.reset_mocap2body_xpos(self.sim)
        obj_quat = self.sim.data.get_body_xquat('robot0:grip')

        print("target", self.sim.data.get_body_xpos('robot0:mocap'))
        print("actual",self.sim.data.get_site_xpos('robot0:grip'))
    ...
    #     # Move x/y randomly
    #     xy = np.clip(np.random.normal(size=2)*np.array([0.07, 0.05]), np.array([-0.15, -0.1]), np.array([0.15, 0.1]))
