...
python -m brl_baselines.run --alg=bppo2_expert --env=Door-v0 --num_timesteps=0 --play --load_path=~/models/maze/bppo_expert_corrective/checkpoints/00001  --num_env=1  --num_trials=100 --output=expert_00001.txt


for i in $(seq -f "%05g" 50 50 4850)
do
	# j=($i % 5)
	# echo $j
	# if [ $j -ne 0 ]; then
