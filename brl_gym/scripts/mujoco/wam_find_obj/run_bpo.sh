...
do
	...
	echo $i
	python -m brl_baselines.run --alg=ppo2 --env=bayes-WamFindObj-v0 --num_timesteps=0 --play --load_path=~/models/maze/bpo/checkpoints/$i  --num_env=1  --num_trials=500 --output=bpo_$i.txt
done
